...
#' algorithm
#'
#' \code{vit_mHMM_pois} obtains the most likely state sequence (for each subject)
#' from an object of either class \code{HMM} or \code{mHMM} (generated by the
#' function \code{mHMM_pois()} for sequences obtained from multiple subjects or animals,
...
#' output object \code{return_path} of the function \code{mHMM_pois()}.
#'
#' @param object An object of class \code{mHMM_pois}, generated by the
#'   function \code{\link{mHMM_pois}}, respectively.
#' @inheritParams mHMM
...
#'
#' @return The function \code{vit_mHMM_pois} returns a matrix containing the most
...
vit_mHMM_pois <- function(object, s_data, burn_in = NULL){
  ...
  est_emiss  <- rep(list(rep(list(matrix(,nrow = m, ncol = 1)),n_dep)), n_subj)
  for(s in 1:n_subj){
    for(q in 1:n_dep){
