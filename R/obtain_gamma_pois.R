...
#' @export
#'
obtain_gamma_pois <- function(object, level = "group", burn_in = NULL){
  ## #ADD CHECK FOR CLASS mHMM_pois
  ...
  if (level == "subject"){
    ...
    for(i in 1:n_subj){
      if(is.mHMM(object)){
        ...
        est_gamma[[i]][] <- matrix(round(apply(object$PD_subj[[i]][burn_in:J, (1 + m * n_dep) : (m * n_dep + m * m)], 2, median), 3),
                                   byrow = TRUE, ncol = m, nrow = m)
      }
    }
  }
