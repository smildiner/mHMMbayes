#' @export
#'
summary.mHMM_pois <- function(object, ...){
  input       <- object$input
  ...
  for(i in 1:n_dep){
    EM_pop[[i]] <- matrix(round(c(apply(object$emiss_alpha_bar[[i]][((burn_in + 1): J),], 2, median), apply(object$emiss_beta_bar[[i]][((burn_in + 1): J),], 2, median)),3), ncol = 2, nrow = m)
    colnames(EM_pop[[i]]) <- c("Shape", "Rate")
