...
#'                               2.0, 0.1), nrow = m, byrow = TRUE))
#'
#' data_cont <- sim_mHMM(n_t = n_t, n = n, m = m, n_dep = n_dep, data_distr = 'poisson',
#'                   gamma = gamma, emiss_distr = emiss_distr, var_gamma = .5, var_emiss = NULL)
#'
  ...
  for(j in 1:n){
    ...
    for(i in 1:n_dep){
      if(data_distr == "categorical"){
        ...
        sub_emiss[[j]][[i]] <- matrix(, nrow = m, ncol = 1)
        for(r in 1:m){
          sub_emiss[[j]][[i]][r,1] <- rgamma(1, shape = emiss_distr[[i]][r,1], rate = emiss_distr[[i]][r,2])
        }
      }
    }
